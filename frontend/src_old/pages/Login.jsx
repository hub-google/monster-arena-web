    ...
    <div className="min-h-screen flex items-center justify-center p-6 bg-gradient-to-br from-slate-950 via-slate-900 to-cyan-950 font-sans relative overflow-hidden">
      ...
      <div className="relative w-full max-w-md glass-panel p-8 flex flex-col items-center">
        ...
        <form onSubmit={handleSubmit} className="w-full flex flex-col gap-5">
          ...
          <div>
            <label className="flex justify-between items-center mb-2 pl-1">
              <span className="block text-xs font-bold uppercase tracking-wider text-cyan-500">密碼 (Password)</span>
              {!isRegister && (
                <button
                  type="button"
                  onClick={handleForgotPassword}
                  className="text-xs font-bold text-cyan-400 hover:text-cyan-300 transition-colors underline decoration-cyan-400/50 underline-offset-2"
                >
                  忘記密碼？
                </button>
              )}
          ...
          {resetSent && (
            <div className="w-full bg-emerald-500/10 border border-emerald-500/30 rounded-xl p-3 text-sm text-emerald-400 text-center font-bold">
              ✅ 密碼重置信件已發送！請檢查您的信箱（包含垃圾郵件匣）。
            </div>
          )}
